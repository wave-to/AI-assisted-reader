...
Future<bool> performStorageMigration({
  ...
  try {
    for (int i = 0; i < dataFolders.length; i++) {
      ...
      AarLog.info('StorageMigration: Copied $folder successfully');
    }

    // Also copy the log file if it exists
    onProgress?.call('aar_reader.log', 6, totalItems);
    final sourceLogFile =
        File('$sourcePath${Platform.pathSeparator}aar_reader.log');
    if (sourceLogFile.existsSync()) {
      final destLogFile =
          File('$destinationPath${Platform.pathSeparator}aar_reader.log');
      await sourceLogFile.copy(destLogFile.path);
      AarLog.info('StorageMigration: Copied log file successfully');
    }
    ...
    for (final folder in successfullyMigrated) {
      ...
      if (sourceDir.existsSync()) {
        ...
        AarLog.info('StorageMigration: Deleted source $folder');
      }
    }
    ...
    AarLog.info('StorageMigration: Completed successfully');
    return true;
  } catch (e) {
    AarLog.severe('StorageMigration failed: $e');
    return false;
  }
}
