...
import 'package:ai_assisted_reader/service/convert_to_epub/create_epub.dart';
import 'package:ai_assisted_reader/service/convert_to_epub/section.dart';
...
String readFileWithEncoding(File file) {
  ...
  for (final entry in decoder.entries) {
    try {
      AarLog.info('Convert: Reading file with encoding: ${entry.key}');
      final content = entry.value(file.readAsBytesSync());
      if (!checkGarbled(content)) {
        return content;
      }
      AarLog.info('Convert: Detected garbled text ${entry.key}');
    } catch (e) {
      AarLog.warning(
...
Future<File> convertFromTxt(File file) async {
  ...
  AarLog.info('convert from txt. title: $titleString, author: $authorString');
  ...
  AarLog.info('convert from txt. content: ${content.length}');
  ...
  AarLog.info('matches: ${matches.length}');

  List<Section> sections;
  if (matches.isEmpty) {
    AarLog.info('Convert: No chapters matched, using fallback chunking');
    sections = _fallbackChunking(filename, content);
    AarLog.info('Convert: Created ${sections.length} sections via fallback');
  } else {
    AarLog.info('Convert: Building ${matches.length} sections from matches');
    sections = _buildSectionsFromMatches(
      content: content,
      matches: matches,
      fallbackTitle: filename,
    );
    AarLog.info('Convert: Created ${sections.length} sections');
  }

  AarLog.info('Convert: Starting EPUB creation...');
  try {
    final epubFile = await createEpub(titleString, authorString, sections);
    AarLog.info('Convert: EPUB created successfully at ${epubFile.path}');
    return epubFile;
  } catch (e) {
    AarLog.severe('Convert: Failed to create EPUB: $e');
    rethrow;
  }
}
