...
class WebdavClient extends SyncClientBase {
  ...
  Future<void> ping() async {
    ...
    while (count < 3) {
      try {
        ...
        AarLog.warning('WebDAV ping failed, retrying... ($count)');
        count++;
        if (count >= 3) {
          AarLog.severe('WebDAV ping failed after 3 attempts: $e');
          rethrow;
        }
      }
    }
  ...
  Future<void> testFullCapabilities() async {
    ...
    try {
      AarLog.info('WebDAV full test: Starting comprehensive test');
      ...
      final testContent = 'AI Assisted Reader WebDAV Test\n'
          'Test Time: ${DateTime.now()}\n'
          'Platform: ${AarPlatform.type.name}\n'
          'Timestamp: $timestamp\n';

      await localTestFile.writeAsString(testContent);
      AarLog.info('WebDAV full test: Created local test file');

      // 2. Create remote test directory
      try {
        await mkdirAll(testDir);
        AarLog.info('WebDAV full test: Created remote directory');
      } catch (e) {
        AarLog.severe('WebDAV full test: Failed to create directory: $e');
      ...
      try {
        ...
        AarLog.info('WebDAV full test: Uploaded test file');
      } catch (e) {
        AarLog.severe('WebDAV full test: Failed to upload file: $e');
      ...
      try {
        ...
        AarLog.info('WebDAV full test: Downloaded test file');
        ...
        AarLog.info('WebDAV full test: Content verification passed');
      } catch (e) {
        if (e.toString().contains('content mismatch')) {
          rethrow;
        }
        AarLog.severe('WebDAV full test: Failed to download file: $e');
      ...
      try {
        ...
        AarLog.info('WebDAV full test: Deleted remote test file');
      } catch (e) {
        AarLog.warning('WebDAV full test: Failed to delete test file: $e');
      ...
      try {
        ...
        AarLog.info('WebDAV full test: Deleted test directory');
      } catch (e) {
        AarLog.info(
      ...
      AarLog.info('WebDAV full test: All tests passed successfully');
    } catch (e) {
      ...
      try {
        ...
        AarLog.warning('WebDAV full test: Cleanup error: $cleanupError');
      }
      rethrow;
    }
  }
  ...
  Future<void> uploadFile(
    ...
    if (replace) {
      try {
        ...
        AarLog.severe('Failed to remove file\n$e');
      }
    }
