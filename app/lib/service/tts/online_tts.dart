...
import 'package:ai_assisted_reader/service/tts/tts_service.dart';
import 'package:ai_assisted_reader/service/tts/tts_service_provider.dart';
import 'package:ai_assisted_reader/service/tts/models/tts_segment.dart';
import 'package:ai_assisted_reader/service/tts/models/tts_sentence.dart';
import 'package:ai_assisted_reader/service/tts/models/tts_voice.dart';
...
class OnlineTts extends BaseTts {
  ...
  Future<void> _startPrefetcher() async {
    ...
    try {
      ...
      AarLog.severe('Prefetcher error: $e');
    } finally {
  ...
  Future<List<TtsSentence>> _collectSentences(int count) async {
    ...
    try {
      ...
      AarLog.severe('Collect sentences error: $e');
      return [];
    }
  }
  ...
  Future<void> _fetchAudioForSegment(TtsSegment segment) async {
    ...
    for (var attempt = 0; attempt <= _maxRetries; attempt++) {
      ...
      try {
        ...
        AarLog.severe('Fetch error (attempt ${attempt + 1}): $e');
        if (attempt == _maxRetries) {
          // Check version before marking as silent
          if (segment.fetchVersion == targetVersion) {
            segment.isSilent = true;
    ...
    try {
      while (!_shouldStop) {
        ...
        try {
          ...
          AarLog.severe('Playback error: $e');
        }
      ...
      AarLog.severe('Player loop error: $e');
    } finally {
