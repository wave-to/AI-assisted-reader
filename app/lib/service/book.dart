...
import 'package:ai_assisted_reader/dao/theme.dart';
...
import 'package:ai_assisted_reader/models/current_reading_state.dart';
...
import 'package:ai_assisted_reader/service/convert_to_epub/txt/convert_from_txt.dart';
import 'package:ai_assisted_reader/service/md5_service.dart';
import 'package:ai_assisted_reader/utils/webView/aar_headless_webview.dart';
...
import 'package:ai_assisted_reader/utils/import_book.dart';
...
AarHeadlessWebView? headlessInAppWebView;
...
void importBookList(List<File> fileList, BuildContext context, WidgetRef ref) {
  AarLog.info('importBook fileList: ${fileList.toString()}');
...
void _checkDuplicatesAndShowDialog(
  ...
  try {
    ...
    AarLog.severe('MD5 check failed: $e');
    _showImportDialog(
      supportedFiles,
      [],
      {},
                ...
                TextButton(
                    onPressed: () async {
                      ...
                      for (var file in filesToImport) {
                        AarToast.show(path.basename(file.path));
                        setState(() {
                        ...
                        try {
                          ...
                          AarLog.severe('Failed to import ${file.path}: $e');
                          AarLog.severe('Stack trace: $stackTrace');
                          setState(() {
...
Future<void> pushToReadingPage(
  ...
  if (book.isDeleted) {
    AarToast.show(L10n.of(context).bookDeleted);
    return;
  }
  ...
  await Navigator.push(
    ...
    AarLog.info('ReadingPage: poped: ${book.title}');
    currentReading.finish();
    chapterContentBridge.state = null;
    tocSearch.clear();
    AarLog.info('Pop successfully ReadingPage: ${book.title}');
  });
}
...
Future<void> saveBook(
  ...
  AarToast.show(L10n.of(navigatorKey.currentContext!).serviceImportSuccess);
...
Future<void> getBookMetadata(
  ...
  AarLog.info("import start: book url: $bookUrl");

  AarHeadlessWebView webview = AarHeadlessWebView(
