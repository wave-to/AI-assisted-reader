...
void receiveShareIntent(WidgetRef ref) {
  ...
  Future<void> handleShare(SharedMedia? media) async {
    AarLog.info('share: Receive share intent called, ${media?.content}');
    if (media == null ||
        media.attachments == null ||
        media.attachments!.isEmpty) {
      AarLog.info('share: Receive share intent: no media or empty');
      return;
    }
    AarLog.info(
  ...
  handler.sharedMediaStream.listen((SharedMedia media) {
    ...
    AarLog.severe('share: Receive share intent');
  });
