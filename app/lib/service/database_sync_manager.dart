...
class DatabaseSyncManager {
  ...
  static Future<DatabaseSyncResult> safeDownloadDatabase({
    ...
    final cacheDir = AarPlatform.isOhos
    ...
    try {
      AarLog.info('DatabaseSync: Starting safe database download');
      AarLog.info('DatabaseSync: Remote file: $remoteDbFileName');
      AarLog.info('DatabaseSync: Temp file: $tempDbPath');
      ...
      AarLog.info('DatabaseSync: Download completed, starting validation');
      ...
      AarLog.info('DatabaseSync: Created backup at: $backupPath');
      ...
      AarLog.severe('DatabaseSync: Error during safe download: $e');
      await _cleanupTempFile(tempDbPath);
  ...
  static Future<void> _recoverFromBackup(
    ...
    try {
      ...
      AarLog.severe('DatabaseSync: Failed to recover from backup: $e');
      rethrow;
    }
  }
  ...
  static Future<void> _cleanupTempFile(String tempDbPath) async {
    try {
      ...
      if (tempFile.existsSync()) {
        ...
        AarLog.info('DatabaseSync: Cleaned up temp file: $tempDbPath');
      }
    } catch (e) {
      AarLog.warning('DatabaseSync: Failed to cleanup temp file: $e');
    }
  }
    ...
    try {
      ...
      if (backupFiles.length > _maxBackupCount) {
        ...
        for (final file in filesToDelete) {
          await file.delete();
          AarLog.info('DatabaseSync: Cleaned up old backup: ${file.path}');
        }
      }
    } catch (e) {
      AarLog.warning('DatabaseSync: Failed to cleanup old backups: $e');
    }
  }
  ...
  static Future<List<String>> getAvailableBackups() async {
    try {
      ...
      AarLog.warning('DatabaseSync: Failed to get available backups: $e');
      return [];
    }
  }
}
