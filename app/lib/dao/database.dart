import 'dart:async';
import 'dart:io';
import 'package:ai_assisted_reader/utils/get_path/get_cache_dir.dart';
...
class DBHelper {
  ...
  Future<Database> initDB() async {
    ...
    switch (AarPlatform.type) {
      ...
      case AarPlatformEnum.windows:
        ...
        final databasePath = await getAnxDataBasesPath();
        AarLog.info('Database: database path: $databasePath');
        final path = join(databasePath, 'app_database.db');
  ...
  static Future<bool> checkpointWal() async {
    try {
      ...
      AarLog.info('Database: WAL checkpoint completed');
      return true;
    } catch (e) {
      AarLog.warning('Database: WAL checkpoint failed: $e');
      return false;
    }
  }
  ...
  static Future<void> cleanupWalFiles(String dbPath) async {
    try {
      ...
      AarLog.info('Database: WAL files cleaned up');
    } catch (e) {
      AarLog.warning('Database: Failed to cleanup WAL files: $e');
    }
  }
  ...
  static Future<String> prepareUploadSnapshot() async {
    try {
      ...
      try {
        ...
        AarLog.warning('Database: VACUUM INTO failed ($e)');

        // Fallback strategy for platforms with older SQLite versions
        // (SQLite 3.27.0+ required for VACUUM INTO support)
        AarLog.info('Database: Using fallback strategy (Checkpoint+Copy)');
      ...
      AarLog.info('Database: Created snapshot at $snapshotPath');
      ...
      AarLog.severe('Database: Failed to create snapshot: $e');
      rethrow;
    }
  }
    ...
    try {
      ...
      if (needsPatch) {
        ...
        if (bytes.length > 20) {
          ...
          AarLog.info('Database: patched header 18, 19 to 1 successfully');
        }
      }
    } catch (e) {
      AarLog.warning('Database: Failed to patch database header: $e');
    }
  }
  ...
  Future<void> onUpgradeDatabase(
    ...
    AarLog.info('Database: upgrade database from $oldVersion to $newVersion');
    switch (oldVersion) {
      case 0:
        AarLog.info('Database: create database version $newVersion');
      ...
      case 1:
        ...
        // remove '/data/user/0/com.anxcye.aar_reader/app_flutter/' from file_path & cover_path
        await db.execute(
            "UPDATE tb_books SET file_path = REPLACE(file_path, '/data/user/0/com.anxcye.aar_reader/app_flutter/', '')");
        await db.execute(
            "UPDATE tb_books SET cover_path = REPLACE(cover_path, '/data/user/0/com.anxcye.aar_reader/app_flutter/', '')");
        continue case2;
      case2:
      case 2:
