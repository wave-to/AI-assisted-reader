...
import 'package:ai_assisted_reader/utils/sync_test_helper.dart';
...
import 'package:ai_assisted_reader/utils/webdav/test_webdav.dart';
...
const String _prefsBackupFileName = 'aar_shared_prefs.json';
...
class _SyncSettingState extends ConsumerState<SyncSetting> {
  ...
  Future<void> exportData(BuildContext context) async {
    ...
    if (await file.exists()) {
      ...
      String fileName =
          'AarReader-Backup-${DateTime.now().year}-${DateTime.now().month}-${DateTime.now().day}-v3.zip';
      ...
      if (filePath != null) {
        AarLog.info('exportData: Saved to: $filePath');
        AarToast.show(L10n.of(navigatorKey.currentContext!).exportTo(filePath));
      } else {
        AarLog.info('exportData: Cancelled');
        AarToast.show(L10n.of(navigatorKey.currentContext!).commonCanceled);
      }
    }
  }

  Future<void> importData() async {
    AarLog.info('importData: start');
    if (!mounted) return;
    ...
    if (filePath == null) {
      AarLog.info('importData: cannot get file path');
      AarToast.show(
    ...
    if (!await zipFile.exists()) {
      AarLog.info('importData: zip file not found');
      AarToast.show(
          L10n.of(navigatorKey.currentContext!).importCannotGetFilePath);
      return;
    }
    ...
    String extractPath = '$cachePath${pathSeparator}aar_reader_import';
    ...
    try {
      ...
      AarLog.info('importData: import success');
      AarToast.show(
          L10n.of(navigatorKey.currentContext!).importSuccessRestartApp);
    } catch (e) {
      AarLog.info('importData: error while unzipping or copying files: $e');
      AarToast.show(
...
Future<String> createZipFile(Map<String, dynamic> params) async {
  ...
  final zipPath = '${(await getAnxTempDir()).path}/AarReader-Backup-$date.zip';
  ...
  final directoryList = [
    ...
    getFontDir(path: docPath),
    getBgimgDir(path: docPath),
    if (!AarPlatform.isOhos) await getAnxDataBasesDir(),
    // await getAnxSharedPrefsDir(),
    // await getAnxShredPrefsFile(),
    prefsBackupFile,
  ];

  AarLog.info('exportData: directoryList: $directoryList');

  final encoder = ZipFileEncoder();
  encoder.create(zipPath);

  if (AarPlatform.isOhos) {
...
Future<bool> _restorePrefsFromBackup(String extractPath) async {
  ...
  try {
    ...
    AarLog.info('importData: prefs backup has unexpected format');
  } catch (e) {
    AarLog.info('importData: failed to restore prefs backup: $e');
  }
  return false;
}
