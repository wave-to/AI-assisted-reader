...
import 'package:ai_assisted_reader/enums/page_turn_mode.dart';
import 'package:ai_assisted_reader/enums/reading_info.dart';
import 'package:ai_assisted_reader/enums/translation_mode.dart';
import 'package:ai_assisted_reader/enums/writing_mode.dart';
...
import 'package:ai_assisted_reader/models/book_style.dart';
import 'package:ai_assisted_reader/models/bookmark.dart';
import 'package:ai_assisted_reader/models/font_model.dart';
import 'package:ai_assisted_reader/models/read_theme.dart';
import 'package:ai_assisted_reader/models/reading_rules.dart';
import 'package:ai_assisted_reader/models/search_result_model.dart';
import 'package:ai_assisted_reader/models/toc_item.dart';
import 'package:ai_assisted_reader/page/book_player/image_viewer.dart';
...
import 'package:ai_assisted_reader/providers/book_toc.dart';
import 'package:ai_assisted_reader/providers/bookmark.dart';
import 'package:ai_assisted_reader/providers/chapter_content_bridge.dart';
import 'package:ai_assisted_reader/providers/current_reading.dart';
import 'package:ai_assisted_reader/service/book_player/book_player_server.dart';
import 'package:ai_assisted_reader/providers/toc_search.dart';
import 'package:ai_assisted_reader/service/tts/base_tts.dart';
import 'package:ai_assisted_reader/service/tts/models/tts_sentence.dart';
import 'package:ai_assisted_reader/service/tts/tts_handler.dart';
import 'package:ai_assisted_reader/utils/coordinates_to_part.dart';
import 'package:ai_assisted_reader/utils/js/convert_dart_color_to_js.dart';
...
import 'package:ai_assisted_reader/utils/webView/gererate_url.dart';
import 'package:ai_assisted_reader/utils/webView/webview_console_message.dart';
import 'package:ai_assisted_reader/widgets/bookshelf/book_cover.dart';
import 'package:ai_assisted_reader/widgets/context_menu/context_menu.dart';
import 'package:ai_assisted_reader/widgets/reading_page/more_settings/page_turning/diagram.dart';
import 'package:ai_assisted_reader/widgets/reading_page/more_settings/page_turning/types_and_icons.dart';
import 'package:ai_assisted_reader/widgets/reading_page/style_widget.dart';
...
class EpubPlayerState extends ConsumerState<EpubPlayer>
  ...
  // Swipe gesture tracking (JS-injected, prevents double-trigger with click)
  bool _swipeJustHandled = false;
  ...
  Future<void> _handleExternalLink(dynamic rawLink) async {
    ...
    if (uri == null || uri.scheme.isEmpty || uri.scheme == 'javascript') {
      AarLog.warning('Ignored invalid external link: $link');
      return;
    }
    ...
    if (!opened) {
      AarLog.warning('Failed to open external link: $link');
    }
  }
  ...
  Future<void> setHandler(InAppWebViewController controller) async {
    ...
    controller.addJavaScriptHandler(
        handlerName: 'onClick',
        callback: (args) {
          // Skip click handling if a swipe was just processed
          if (_swipeJustHandled) {
            _swipeJustHandled = false;
            return;
          }
          Map<String, dynamic> location = args[0];
          onClick(location);
    ...
    controller.addJavaScriptHandler(
        handlerName: 'onSwipe',
        callback: (args) {
          final direction = args[0] as String;
          _swipeJustHandled = true;
    ...
    controller.addJavaScriptHandler(
      ...
      callback: (args) async {
        try {
          ...
          AarLog.severe('Translation error: $e');
  ...
  Future<void> onWebViewCreated(InAppWebViewController controller) async {
    ...
    _injectSwipeDetection(controller);
  ...
  /// Inject JavaScript that detects horizontal swipe gestures inside the
  /// WebView and calls back to Flutter via window.flutter_inappwebview.
  /// This is needed because hybrid composition puts the native WebView on
  /// top, preventing Flutter-level gesture detectors from receiving events.
  void _injectSwipeDetection(InAppWebViewController controller) {
    controller.evaluateJavascript(source: '''
      (function() {
        if (window.__swipeDetectionInstalled) return;
        window.__swipeDetectionInstalled = true;
        ...
        document.addEventListener('touchend', function(e) {
          ...
          var dy = Math.abs((e.changedTouches[0]?.clientY || 0) - startY);
          var dt = Date.now() - startTime;
          var threshold = 50;
          // Must be: significant horizontal movement, dominant over vertical,
          // and fast enough (under 500ms)
          if (Math.abs(dx) > threshold && Math.abs(dx) > dy * 1.8 && dt < 500) {
