...
import 'package:ai_assisted_reader/models/remote_file.dart';
import 'package:ai_assisted_reader/models/sync_state_model.dart';
import 'package:ai_assisted_reader/providers/book_list.dart';
import 'package:ai_assisted_reader/providers/sync_status.dart';
import 'package:ai_assisted_reader/providers/tb_groups.dart';
import 'package:ai_assisted_reader/service/sync/sync_client_factory.dart';
import 'package:ai_assisted_reader/service/sync/sync_client_base.dart';
import 'package:ai_assisted_reader/service/database_sync_manager.dart';
import 'package:ai_assisted_reader/dao/database.dart';
import 'package:ai_assisted_reader/utils/get_path/databases_path.dart';
...
class Sync extends _$Sync {
  ...
  Future<void> init() async {
    ...
    if (client == null) {
      AarLog.severe('No sync client configured');
      return;
    }

    AarLog.info('${client.protocolName}: init');
  }
  ...
  Future<bool> shouldSync() async {
    ...
    if (Prefs().onlySyncWhenWifi &&
      ...
      if (Prefs().syncCompletedToast) {
        AarToast.show(L10n.of(navigatorKey.currentContext!).webdavOnlyWifi);
      }
      return false;
    }
  ...
  Future<SyncDirection?> determineSyncDirection(
    ...
    AarLog.info('localDbTime: $localDbTime, remoteDbTime: ${remoteDb?.mTime}');
  ...
  Future<SyncDirection?> _showSyncDirectionDialog(
    ...
    if (_isShowingDirectionDialog) {
      AarLog.info('Sync direction dialog already showing, skipping');
      return null;
    }
  ...
  Future<void> syncData(
    ...
    final client = _syncClient;
    if (client == null) {
      AarLog.info('No sync client configured');
      return;
    }
    ...
    if (state.isSyncing) {
      AarLog.info('Sync already in progress, skipping');
      return;
    }
    ...
    try {
      ...
      AarLog.severe('Sync connection failed, ping failed2\n${e.toString()}');
      return;
    }

    AarLog.info('Sync ping success');
    ...
    if (Prefs().syncCompletedToast) {
      AarToast.show(L10n.of(navigatorKey.currentContext!).webdavSyncing);
    }
    ...
    try {
      ...
      if (Prefs().syncCompletedToast) {
        AarToast.show(L10n.of(navigatorKey.currentContext!).webdavSyncingFiles);
      }
      ...
      try {
        ...
        AarLog.info('Failed to refresh book list: $e');
      }

      // Backup cleanup is now handled by DatabaseSyncManager

      if (Prefs().syncCompletedToast) {
        AarToast.show(L10n.of(navigatorKey.currentContext!).webdavSyncComplete);
      }
    } catch (e, s) {
      if (e is DioException && e.type == DioExceptionType.connectionError) {
        AarToast.show('Sync connection failed, check your network');
        AarLog.severe('Sync connection failed, connection error\n$e, $s');
      } else {
        AarToast.show('Sync failed\n$e');
        AarLog.severe('Sync failed\n$e, $s');
      }
    } finally {
      changeState(state.copyWith(isSyncing: false));
      // _deleteBackUpDb();
  ...
  Future<void> syncDatabase(SyncDirection direction) async {
    ...
    try {
        ...
        case SyncDirection.download:
          if (remoteDb != null) {
            ...
            if (!result.isSuccess) {
              ...
              AarLog.severe('Database sync failed: ${result.message}');
      ...
      AarLog.severe('Failed to sync database\n$e');
      rethrow;
    }
  }
  ...
  Future<void> downloadBook(Book book) async {
    ...
    if (!syncStatus.remoteOnly.contains(book.id)) {
      AarToast.show(L10n.of(navigatorKey.currentContext!)
  ...
  Future<void> releaseBook(Book book) async {
    ...
    Future<void> uploadBook() async {
      try {
        ...
        AarLog.severe('Failed to upload book\n$e');
        rethrow;
      }
    }
  ...
  Future<void> downloadMultipleBooks(List<int> bookIds) async {
    ...
    for (final bookId in bookIds) {
      try {
        ...
        AarLog.info('WebDAV: Downloading book ID $bookId: ${book.title}');
        await _downloadBook(book);
        successCount++;
      } catch (e) {
        AarLog.severe('WebDAV: Failed to download book ID $bookId: $e');
        failCount++;
      }
    }

    AarLog.info(L10n.of(navigatorKey.currentContext!)
  ...
  Future<void> _downloadBook(Book book) async {
    try {
      ...
      AarLog.severe('Failed to download book\n$e');
      rethrow;
    }
  }
  ...
  Future<void> showBackupManagementDialog() async {
    try {
      ...
      AarLog.severe('Failed to show backup management dialog: $e');
      AarToast.show('Failed to get backup list: $e');
    }
  }
  ...
  Future<void> _restoreFromBackup(String backupPath) async {
    try {
      ...
      try {
        ...
        AarLog.info('Failed to refresh providers after restore: $e');
      }

      AarToast.show(L10n.of(navigatorKey.currentContext!).restoreSuccess);
      AarLog.info('Database restored from backup: $backupPath');
    } catch (e) {
      AarLog.severe('Failed to restore from backup: $e');
      AarToast.show('Restore failed: $e');
    }
  }
